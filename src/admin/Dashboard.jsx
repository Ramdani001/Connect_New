...
export default function Dashboard(props){
    ...
    const getTrans = async () => {
        try {
            ...
            const ress = await axios.get("http://localhost:3000/api/v1/count");
    ...
    const statChange = async() => {
        ...
        const response = await axios.get(`http://localhost:3000/api/v1/transaksi/two/${idT}`);
    ...
    const handleAction = async(data) => {
        ...
        const response = await axios.get(`http://localhost:3000/api/v1/transaksi/one/${id}`);
        ...
        if(response.status == 200){
            ...
            if (imgElement && fileName) {
                ...
                img_bukti_zoom.src = `images/products/${fileName}`;
            }

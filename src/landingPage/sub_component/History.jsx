...
export default function History(props){
    ...
    const getMessages = async () => {
        ...
        try {
            const res = await axios.get(`http://www.tech-in-dynamic.site:3000/api/v1/messages/custSess/${id_user}`);
            // setArrMess(res.data);
    ...
    const handleMessage = async (e) => {
        ...
        try {
            ...
            if(id_m){
                ...
                if(response){
                    const res = await axios.get(`http://www.tech-in-dynamic.site:3000/api/v1/messages/mess/${id_m}`);
                    setArrMess(res.data);
                    document.getElementById("send_mess").value = "";
                }
            }else{
                const res = await axios.get(`http://www.tech-in-dynamic.site:3000/api/v1/messages/newInsert/${id_user}`);
    ...
    const sendData = async(data) => {
        const response = await axios.post(`http://www.tech-in-dynamic.site:3000/api/v1/messages/insert/`, data);
                
        if(response){
            const res = await axios.get(`http://www.tech-in-dynamic.site:3000/api/v1/messages/mess/${data.id_m}`);
            setArrMess(res.data);
            document.getElementById("send_mess").value = "";
        }
    }
